...
> **dev モードで想定される挙動:** `docker compose logs gateway` は起動の度に黄色い
> `WARN` を 3 行出力します — `Building OS link is plaintext h2c
> (--bos-insecure)`・`catalog: cosign verification disabled`・`admin: JWT auth
> disabled`。この認証なし・TLS なしの dev スタックでは 3 つとも意図的なものです
> (ADR-0006/ADR-0007)。ラボ以外へのデプロイ前に [SECURITY.md](../SECURITY.md)
> を参照してください。

`gateway` サービスは組み込みの **sim コネクタ**(`sim-01`、dev/CI 専用)も既定で起動するため、
スタックが healthy になった時点で既にテレメトリが流れています — 確認方法は下記 §3/§5 参照。
本ガイドが約束する「約10分、機器なし」の体験に追加設定は不要です。
...
組み込みの `sim-01` コネクタ(§2)が既に動作しているため、以下のコマンドはどれも
追加設定なしで実データを返します。
...
> **Catalog からの Install はそのままでは動きません。** 同梱の `fixtures/catalog.json`
> はプレースホルダの digest(`sha256:0000…0000`)しか持たず、実在し到達可能な OCI
> レジストリに署名済みイメージを実際に公開しない限り Install は必ず失敗します。これは
> dev fixture としての既定挙動であり、ローカルで回避すべきバグではありません。レジストリ
> 不要で動くコネクタが欲しい場合は、組み込みの `sim-01`(§2)か、MQTT で自分の
> Device/Point を追加する方法(§7)を使ってください。

---

## 6. ゲートウェイを直接実行(機器なし・Docker なし)

§2 の compose スタックは既に sim コネクタを動かしています — 本節は **Go コードそのものを
素早く反復したい場合**向けです: Docker イメージの再ビルドなしに、ベアな NATS に対して
...
> **セットアップ不要ですぐ動く例が欲しい場合:** §8 の [MQTT](#mqtt) サブセクションは
> 既にバンドル済みブローカーと対応する Point List エントリ
> (`room1_temperature`/`room1_setpoint`)を同梱しています — 外部ブローカーも手動
> 編集も不要です。本ウォークスルーはそれとは別に、自分の**新しい** Device/Point を
> オンボードする*一般的な*手順を、§8 の既定例と衝突しない別の例
> (`lobby_temperature`)で教えるものです — 命名が §8 と異なるのは意図的です。
...
(例: ローカルの [Mosquitto](https://mosquitto.org/))に向けます — 自分のブローカーが
無ければ、外部インフラを立てる代わりに §8 の [MQTT](#mqtt) サブセクションの
バンドル済みブローカーを再利用できます:

```bash
# 自分のブローカーが無ければ、まずバンドル済みのものだけ起動:
...
トピックに値を publish します(コネクタが Common Event に正規化します)。ブローカーの
ホストは適宜置き換えてください — Step 2 でバンドル済みのものを再利用した場合は
`localhost -p 11883`:
