...
- **Building OS contract is authoritative over design doc §8.** Real proto lives in `../gutp-building-os-ri/proto/` (package `gatewaybridge`). Ingress = `GatewayIngress/StreamTelemetry` with typed number/string/boolean telemetry; Egress = `GatewayEgress/Connect`, including the optional Point List revision status heartbeat. No DiscoveryService.
...
- **Superseded by Building OS #152 / ADR-0006:** `TelemetryFrame.value` is `oneof { value_num = 3; value_str = 6; value_bool = 7; }`. Field 3 preserves numeric wire compatibility. `timestamp` is RFC3339 and carries source observation time.
...
- **The MQTT connector is Go; the Modbus connector is planned (Go), not yet implemented.** paho.golang (native MQTT 5.0); toolchain/SDK assets shared with the Core Agent; minimal images. When Modbus lands it will follow the same Go pattern. BACnet stays Python only because of BACpypes3, OPC-UA stays Java only because of Eclipse Milo — stack-driven, not policy.
