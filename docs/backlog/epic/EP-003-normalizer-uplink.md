...
The contract is **owned by Building OS** (`../gutp-building-os-ri/proto/`, package `gatewaybridge`): `GatewayIngress/StreamTelemetry`, client-streaming, ack on stream close. There is **no DiscoveryService** and the gateway authors no public proto. Delivery is **best-effort** (ADR-0002), not no-loss. Typed-value alignment with the current Building OS contract is tracked in EP-012.

## Acceptance Criteria

- [ ] Normalizer is the single durable pull consumer on `evt.>` of the `EVENTS` stream (ADR-0005); it joins the Point List to resolve `local_id` → canonical `point_id`, unifies unit/quality/timestamp, and produces `TelemetryFrame{gateway_id, point_id, oneof value, RFC3339 timestamp, attributes}`.
- [ ] `value` preserves number/string/boolean as `value_num`/`value_str`/`value_bool` under the current Building OS #152 / ADR-0006 contract; implementation and compatibility coverage are tracked in EP-012.
- [ ] Semantic mapping (REC/Brick/QUDT/BOT) is hosted in the Normalizer (interface stubbed for MVP), never in connectors.
- [ ] `gatewaybridge` protos are vendored from `gutp-building-os-ri` and managed with Buf, including breaking-change detection against the vendored baseline.
...
- EP-012 Building OS Contract Alignment — updates this pipeline from the superseded numeric-only contract to typed telemetry.
